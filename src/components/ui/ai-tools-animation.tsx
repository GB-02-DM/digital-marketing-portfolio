...
export const Skeleton = () => {
  ...
  return (
    <div className="p-4 overflow-hidden h-full relative flex items-center justify-center">
      ...
      <div className="h-full w-[3px] absolute inset-0 m-auto z-10 bg-gradient-to-b from-transparent via-cyan-500 to-transparent animate-move shadow-[0_0_10px_#22d3ee]">
        <div className="w-20 h-40 top-1/2 -translate-y-1/2 absolute -left-10">
          <Sparkles />
        </div>
      </div>
...
const Sparkles = () => {
  ...
  const randomOpacity = () => Math.random() * 0.7 + 0.3; // Ensure minimum opacity of 0.3
      ...
      {[...Array(24)].map((_, i) => (
        <motion.span
          ...
          style={{
            ...
            top: `${random() * 100}%`,
            left: `${random() * 100}%`,
            width: `${Math.max(2, random() * 4)}px`,
            height: `${Math.max(2, random() * 4)}px`,
            borderRadius: "50%",
            zIndex: 1,
            backgroundColor: i % 3 === 0 ? '#4ade80' : i % 3 === 1 ? '#22d3ee' : '#ffffff',
            boxShadow: `0 0 ${Math.max(2, random() * 4)}px ${i % 3 === 0 ? '#4ade80' : i % 3 === 1 ? '#22d3ee' : '#ffffff'}`,
          }}
